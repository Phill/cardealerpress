...
class wp_auto_update
{
	...
	public function getRemote_data(){
		...
		$request = new http_request( $this->update_path, 'cardealerpress_plugin_updater' );
    ...
    public function display_changelog(){
		...
		$request = new http_request( 'http://updates.s3.dealertrend.com/wp-plugin-inventory-api/changelog.html', 'cardealerpress_plugin_changelog' );
		$request = $request->cached() ? $request->cached() : $request->get_file();

		if (is_wp_error($request) || 200 != wp_remote_retrieve_response_code($request) ) {
            $page_text = sprintf(__("Connection lost.%sPlease try again or %scontact support%s.", 'cardealerpress'), "<br/>", "<a href='http://www.dealertrend.com'>", "</a>");
        }else{
