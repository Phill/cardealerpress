<?php
	...
	function generate_inventory_link($rule, $params, $include = array(), $remove = array(), $type = 0 ){
		...
		if($rule){
			if( $type ){
				$link = '/inventory/'.$params['year'].'/'.rawurlencode($params['make']).'/'.rawurlencode($params['model']).'/'.rawurlencode($params['state']).'/'.rawurlencode(strip_tags($params['city'])).'/'.$params['vin'].'/';
			} else {
